...
<div style="max-width:900px;margin:0 auto;padding:28px 18px;position:relative;">
    ...
    <div style="margin-top:26px;background:#fff;border:1px solid #e5e7eb;border-radius:18px;padding:18px;">
        ...
        @if (session('success'))
            <div style="margin-top:14px;padding:10px;background:#ecfdf5;border:1px solid #bbf7d0;border-radius:10px;">
                {{ session('success') }}
            </div>
        @endif
        ...
        <form method="POST" action="{{ route('profile.update') }}" style="margin-top:16px;">
            ...
            <input name="name" value="{{ old('name', $user->name) }}"
                   style="width:100%;box-sizing:border-box;padding:10px;border:1px solid #d1d5db;border-radius:12px;margin:6px 0 12px;">
    ...
    <!-- Posts section -->
    <div style="margin-top:18px;background:#fff;border:1px solid #e5e7eb;border-radius:18px;padding:18px;">
        <div style="color:#f97316;font-weight:900;letter-spacing:1px;font-size:20px;margin-bottom:12px;">
            POSTS
        </div>

        <div style="max-height:55vh;overflow-y:auto;padding-right:6px;">
            @forelse (($posts ?? []) as $post)
                <div style="border:1px solid #e5e7eb;border-radius:14px;padding:14px;margin-bottom:12px;background:#fafafa;">
